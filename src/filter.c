...
FilterContext *filter_context_create(VideoContext *vc) {
	...
	snprintf(args, sizeof(args),
			 ...
			 vc->cod_ctx->sample_aspect_ratio.num,
			 vc->cod_ctx->sample_aspect_ratio.den);

	printf("Args:\n%s\n", args);

	if (avfilter_graph_create_filter(&fc->buffersrc_ctx, fc->buffersrc, "in",
									 args, NULL, fc->filtergraph) < 0) {
		fprintf(stderr, "Failed to create buffer source\n");
		return NULL;
	}
	if (avfilter_graph_create_filter(&fc->buffersink_ctx, fc->buffersink, "out",
									 NULL, NULL, fc->filtergraph) < 0) {
		fprintf(stderr, "Failed to create buffer sink\n");
		return NULL;
	}
	enum AVPixelFormat pix_fmts[] = {AV_PIX_FMT_YUV420P, AV_PIX_FMT_NONE};
	if (av_opt_set_int_list(fc->buffersink_ctx, "pix_fmts", pix_fmts,
							AV_PIX_FMT_YUV420P, AV_OPT_SEARCH_CHILDREN) < 0) {
...
AVFrame *filter_context_exec(FilterContext *fc, AVFrame *frame,
	...
	if (avfilter_graph_parse_ptr(fc->filtergraph, filter_desc, &fc->inputs,
								 &fc->outputs, NULL) < 0) {
	...
	if (av_buffersrc_add_frame_flags(fc->buffersrc_ctx, frame,
