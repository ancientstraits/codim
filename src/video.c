...
VideoContext *video_context_create(const char *video_path, size_t width,
	...
	if (avcodec_parameters_from_context(vc->stream->codecpar, vc->cod_ctx) <
		0) {
...
int video_context_should_update(VideoContext *vc) {
	return !(av_compare_ts(vc->pts, vc->cod_ctx->time_base, vc->duration,
						   (AVRational){1, 1}) < 0);
}
...
int video_context_write_frame(VideoContext *vc) {
	...
	while (ret >= 0) {
		...
		av_packet_rescale_ts(vc->pkt, vc->cod_ctx->time_base,
							 vc->stream->time_base);
